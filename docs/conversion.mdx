...
+----------------------+------------------------+------------------------------------------+-----------------------------+------------------------+------------------------+
| Universal term       | Claude                 | Codex (app-server)                       | OpenCode                    | Amp                    | Copilot                |
+----------------------+------------------------+------------------------------------------+-----------------------------+------------------------+------------------------+
| session_id           | n/a (daemon-only)      | n/a (daemon-only)                        | n/a (daemon-only)           | n/a (daemon-only)      | n/a (daemon-only)      |
| native_session_id    | none                   | threadId                                 | sessionID                   | none                   | data.sessionId         |
| item_id              | synthetic              | ThreadItem.id                            | Message.id                  | StreamJSONMessage.id   | data.messageId         |
| native_item_id       | none                   | ThreadItem.id                            | Message.id                  | StreamJSONMessage.id   | data.messageId         |
+----------------------+------------------------+------------------------------------------+-----------------------------+------------------------+------------------------+
...
| Universal term         | Claude                       | Codex (app-server)                         | OpenCode                                | Amp                              | Copilot                                  |
+------------------------+------------------------------+--------------------------------------------+-----------------------------------------+----------------------------------+------------------------------------------+
| session.started        | none                         | method=thread/started                      | type=session.created                    | none                             | type=session.start                       |
| session.ended          | SDKMessage.type=result       | no explicit session end (turn/completed)   | no explicit session end (session.deleted)| type=done                        | type=session.idle (implicit)             |
| message (user)         | SDKMessage.type=user         | item/completed (ThreadItem.type=userMessage)| message.updated (Message.role=user)    | type=message                     | type=user.message                        |
| message (assistant)    | SDKMessage.type=assistant    | item/completed (ThreadItem.type=agentMessage)| message.updated (Message.role=assistant)| type=message                  | type=assistant.message                   |
| message.delta          | stream_event (partial) or synthetic | method=item/agentMessage/delta      | type=message.part.updated (delta)       | synthetic                        | type=assistant.message_delta             |
| tool call              | type=tool_use               | method=item/mcpToolCall/progress           | message.part.updated (part.type=tool)   | type=tool_call                   | type=tool.execution_start                |
| tool result            | user.message.content.tool_result | item/completed (tool result ThreadItem variants) | message.part.updated (part.type=tool, state=completed) | type=tool_result     | type=tool.execution_complete             |
| permission.requested   | control_request.can_use_tool | none                                      | type=permission.asked                   | none                             | JSON-RPC callback                        |
| permission.resolved    | daemon reply to can_use_tool | none                                      | type=permission.replied                 | none                             | JSON-RPC callback response               |
| question.requested     | tool_use (AskUserQuestion)  | experimental request_user_input (payload) | type=question.asked                     | none                             | ask_user tool call                       |
| question.resolved      | tool_result (AskUserQuestion) | experimental request_user_input (payload) | type=question.replied / question.rejected | none                          | ask_user tool result                     |
| error                  | SDKResultMessage.error       | method=error                               | type=session.error (or message error)   | type=error                        | type=session.error                       |
+------------------------+------------------------------+--------------------------------------------+-----------------------------------------+----------------------------------+------------------------------------------+
...
- Copilot emits assistant.message_delta and assistant.reasoning_delta events natively.
